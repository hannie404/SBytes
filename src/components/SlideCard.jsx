...
function SlideCard({ laptops, title, currentUser }) {
  const navigate = useNavigate();
  const user = currentUser;
  const USformatter = new Intl.NumberFormat("en-US"); // for formatting the price
  const featured = laptops.filter((prod) => {
    return prod.Price < 1000000000;
  });
  const addToCart = (data) => {
    navigate("/CartCard", {state : [data,user]});
  };
            ...
            featured.map((laptop, index) => (
              <SbytesSlide key={index}>
                <Card className="laptop-card text-light d-flex flex-column align-items-center justify-content-center ps-lg-5 pe-lg-5 ps-md-3 pe-md-3">
                  <SBytesImg
                    src={`http://localhost:8000/storage/products/images/${laptop.ImageUrl}`}
                  />
                  <Name className="mb-3">
                    {laptop.ProductSeriesName} {laptop.ProductModel}
                  </Name>
                  <p>
                    <ul>
                      <li>
                        <b>Operating System</b> : {laptop.OperatingSystem}
                      </li>
                      <li>
                        <b>CPU</b> : {laptop.Cpu}
                      </li>
                      <li>
                        <b>Memory</b> : {laptop.Memory}
                      </li>
                      <li>
                        <b>Storage</b> : {laptop.Storage}
                      </li>
                      <li>
                        <b>Integreated GFX</b> : {laptop.IntegratedGfx}
                      </li>
                      <li>
                        <b>Refresh Rate</b> : {laptop.RefreshRate}
                      </li>
                      <li>
                        <b>Resolution</b> : {laptop.Resolution}
                      </li>
                      <li>
                        <b>Screen Size</b> : {laptop.ScreenSize}"
                      </li>
                    </ul>
                  </p>
                ...
                <Buttons fluid className="d-flex ps-3 pe-3 mt-3">
                  <HeartDiv
                    className="w-25 d-flex align-items-center justify-content-center pt-3 pb-3"
                    type="button"
                    id="addToWish"
                  >
                    <img src={Heart} />
                  ...
                  <CartDiv
                    className="w-75 d-flex align-items-center justify-content-center pt-3 pb-3"
                    type="button"
                    id="addToCart"
